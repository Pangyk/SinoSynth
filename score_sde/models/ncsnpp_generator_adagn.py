...
ResnetBlockEnc = layerspp.ResnetBlockBigGANpp_Adagn_encoder
norm_z = layerspp.z_score
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        ResnetBlockEncoder = functools.partial(ResnetBlockEnc,
                                            act=act,
                                            dropout=dropout,
                                            fir=fir,
                                            fir_kernel=fir_kernel,
        ...
        in_chns = config.num_in_chns
        if progressive_input != 'none':
            input_pyramid_ch = in_chns

        modules.append(conv3x3(1, nf))
        hs_c = [nf]
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                modules.append(ResnetBlockEncoder(in_ch=nf, out_ch=nf))
            ...
            if i_level != num_resolutions - 1:
                if resblock_type == 'ddpm':
                    ...
                    modules.append(ResnetBlockEncoder(down=True, in_ch=nf))

                modules.append(pyramid_downsample(in_ch=nf, out_ch=nf))
    
        modules.append(conv3x3(in_chns, nf))
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                print("layer", i_level, "inch", in_ch, "outch", out_ch)
        ...
        mapping_layers = [dense(2048, config.nz), norm_z(), PixelNorm(),
    ...
    def forward(self, x, time_cond, z):
        ...
        zs = modules[m_idx](x[:, 1:2, :, :])
        m_idx += 1
        for i_level in range(self.num_resolutions):
            # Residual blocks for this resolution
            for i_block in range(self.num_res_blocks):
                zs = modules[m_idx](zs, temb)
                m_idx += 1
